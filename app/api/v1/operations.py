...
def add_income(operation: OperationRequest, db: Session = Depends(get_db),  # DB session via dependency injection
...
def add_expense(operation: OperationRequest, db: Session = Depends(get_db),  # DB session via dependency injection
                current_user: User = Depends(get_current_user)):  # Current user from auth token
...
async def create_transfer(
    ...
    user: User = Depends(get_current_user),  # Current user from auth token
    db: Session = Depends(get_db),  # DB session via dependency injection
):
